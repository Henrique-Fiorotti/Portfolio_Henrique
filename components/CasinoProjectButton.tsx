...
export function CasinoProjectButton() {
  ...
  useLayoutEffect(() => {
    ...
    const reels = Array.from(button.querySelectorAll<HTMLElement>(".casinoButtonReel"));
  ...
  const spin = (showHoverLabel: boolean) => {
    const button = buttonRef.current;
    ...
    if (reducedMotion) {
      gsap.set(reels, { y: showHoverLabel ? "-400%" : "0%" });
      return;
    }
  ...
  return (
    <a
      ref={buttonRef}
      className="button primary casinoButton"
      href="#projetos"
      aria-label="Conhecer projetos"
      onMouseEnter={() => spin(true)}
      onMouseLeave={() => spin(false)}
      onFocus={() => spin(true)}
      onBlur={() => spin(false)}
    >
      <span className="casinoButtonText" aria-hidden="true">
        {Array.from(INITIAL_LABEL).map((character, index) => {
          ...
          const sequence = isSpace
            ? [" ", " ", " ", " ", " ", " ", " ", " ", " "]
            : [
                character,
                SPIN_CHARACTERS[index % SPIN_CHARACTERS.length],
                SPIN_CHARACTERS[(index + 1) % SPIN_CHARACTERS.length],
                SPIN_CHARACTERS[(index + 2) % SPIN_CHARACTERS.length],
                hoverCharacter,
                SPIN_CHARACTERS[(index + 2) % SPIN_CHARACTERS.length],
                SPIN_CHARACTERS[(index + 1) % SPIN_CHARACTERS.length],
                SPIN_CHARACTERS[index % SPIN_CHARACTERS.length],
          ...
          return (
            <span className="casinoButtonSlot" key={`${character}-${index}`}>
              <span className="casinoButtonReel">
                {sequence.map((item, sequenceIndex) => (
                  <span key={sequenceIndex}>{item === " " ? "\u00a0" : item}</span>
                ))}
              </span>
            </span>
          );
